...
class Player extends Phaser.Physics.Arcade.Sprite {
    ...
    init() {
        ...
        this.cursers = this.scene.input.keyboard.createCursorKeys()
    ...
    update() {
        ...
        if (this.getBounds().top > this.scene.config.height + 150) {
            EventEmitter.emit('PLAYER_LOOSE')
            return
        }

        const { left, right, space, up } = this.cursers
